 ...
 * Detects the heading "— Risk Assessment" present in every generated report.
 */
function isGeneratedReport(adfBody) {
  if (!adfBody?.content) return false;
  return adfBody.content.some(node =>
    node.type === 'heading' &&
    node.content?.some(c => c.text?.includes('Risk Assessment'))
  );
}
...
function buildCommentAdf(report) {
  ...
  if (report.classification === 'NON-DEFECT') {
    ...
    return {
      version: 1, type: 'doc',
      content: [
        heading(3, `${report.bug_id} \u2014 Risk Assessment`),
